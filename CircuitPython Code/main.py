...
def setuptoggles():  # By storing the state of each key, we can avoid pressing the same key over and over again unintentionally.
	rowlength = len(Columns)
	columnlength = len(Rows)
	Setup = []
	for each in range(rowlength * columnlength):
		Setup.append(False)  # Each key starts in the state of un-pressed.
	return Setup
